...
API = AMPAPIHandler(baseUri=f"/", username="", password="")
API.initHandler()
...
class GameServersCog(commands.Cog):
    ...
    async def mcserverstatus(self, ctx):
        ...
        await ctx.channel.send(
            "The server is online with {0} players and replied in {1}ms".format(
    ...
    async def mcserverstart(self, ctx):
        ...
        if role in ctx.author.roles:
            API.Core.Start()
            await ctx.send(
                "The Minecraft server is starting, please allow a moment or two before attempting to connect."
            )
    ...
    async def mcserverstop(self, ctx):
        ...
        if role in ctx.author.roles:
            API.Core.Stop()
            await ctx.send(
                "The Minecraft server is stopping, please allow a moment for all functions to come to a halt and world to properly save."
            )
...
async def setup(bot):
    await bot.add_cog(GameServersCog(bot))
